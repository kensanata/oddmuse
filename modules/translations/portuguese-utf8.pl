...
Diferença (relativamennte à %s revisão)
major
importante
minor
simples
...
Só os administradores podem criar a página %s.
Transfer Error: %s
...
Falta o parâmetro de ID.
Thread %s does not exist.
Thread %s não existe
Page %s does not contain a thread.
A página %s não contém thread
Add
Adicionar
URL parameter is missing.
Falta o parâmetro de URL.
Add to %s thread
Adicionar ao thread %s
Below:
Abaixo:
URL:
URL:
