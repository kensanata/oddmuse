...
sub named_entity_regular_expression {
...
EOF
  my $entities = decode_json $json;
  my $re = join("|", map { substr($_, 1, -1); } keys %$entities);
  my $reverse_re = join("|", map { quotemeta($entities->{$_}->{characters}); } keys %$entities);
  my %map = map {
    $_ => $entities->{$_}->{characters};
  } keys %$entities;
  my %reverse_map = map {
    $entities->{$_}->{characters} => $_;
  } sort keys %$entities;
  return qr"(&(?:$re);)", \%map, qr"($reverse_re)", \%reverse_map;
}
...
my ($entity_re, $entity_map, $reverse_entity_re, $reverse_entity_map)
    = named_entity_regular_expression();

# print $reverse_entity_re . "\n";
# $_ = QuoteHtml(q{[link](foo%20b&auml;)});
# print "$_\n";
# m/$entity_re/;
# print "Match: $1\n";
# print "That is: " . $entity_map->{$1} . "\n";
# print use_named_entities(qq{"});
# exit;

# Brackets are allowed in the link text only if (a) they are backslash-escaped
# or (b) they appear as a matched pair of brackets, with an open bracket [, a
...
sub valid_link {
  ...
  my $ticks = 0;
  my $angled = 0;
  for (my $i = 0; $i < length($link); $i++) {
    my $c = substr($link, $i, 1);
    if ($c eq '[') {
      $brackets++;
    } elsif ($c eq ']') {
      $brackets--;
      ...
      $ticks = !$ticks;
	     ...
	     and (substr($link, $i + 1, 1) eq '['
		  or substr($link, $i + 1, 1) eq ']')) {
	$i++; # skip next
    } elsif ($c eq '&'
	     and (substr($link, $i + 1, 4) eq '&lt;')) {
      $angled++;
    } elsif ($c eq '&'
	     and (substr($link, $i + 1, 4) eq '&gt;')) {
      $angled--;
  ...
  return $brackets == 0 and $angled <= 0 and not $ticks;
}

sub unbalanced_parentheses {
  my $str = shift;
  my $parens = 0;
  for (my $i = 0; $i < length($str); $i++) {
    ...
    if ($c eq '(') {
      ...
      $parens--;
	     ...
	     and (substr($str, $i + 1, 1) eq '('
		  or substr($str, $i + 1, 1) eq ')')) {
	$i++; # skip next
    }
    if ($parens < 0) {
      return 0;
...
sub encode {
  my $bytes = encode_utf8(shift);
  $bytes =~ s/([^[:alnum:]._~\-:!*'();@&=+\$,\/?#])/sprintf("%%%02X", ord($1))/ge;
  return $bytes;
}

sub encode_without_url_escapes {
  my $str = UnquoteHtml(shift);
  # Entity and numerical character references in the destination will be parsed
  # into the corresponding Unicode code points, as usual.
  $str =~ s/$entity_re/$entity_map->{$1}/g;
  ...
  $bytes =~ s/([^[:alnum:]._~\-:!*'();@&=+\$,\/?#%])/sprintf("%%%02X", ord($1))/ge;
  # If the percent is not a valid URL escape, escape the percent.
  $bytes =~ s/%(?![0-9a-f][0-9a-f])/%25/gi;
  return $bytes;
}

sub use_named_entities {
  my $str = UnquoteHtml(shift);
  # Use named entities where possible. To prevent double encoding of existing
  # named entities (&quot; turning into &amp;quot&semi;), translate those, back,
  # first.
  $str =~ s/$entity_re/$entity_map->{$1}/g;
  $str =~ s/$reverse_entity_re/$reverse_entity_map->{$1}/g;
  return $str;
}
...
sub CommonMarkdownRule {
  ...
  # enclosing <...> if present, with backslash-escapes in effect. A link
  # destination consists of either a sequence of zero or more characters between
  # an opening < and a closing > that contains no spaces, line breaks, or
  # unescaped < or > characters, or a nonempty sequence of characters that does
  # not include ASCII space or control characters, and includes parentheses only
  # if (a) they are backslash-escaped or (b) they are part of a balanced pair of
  ...
  # should be supported.). The link’s title consists of the link title,
  # excluding its enclosing delimiters, with backslash-escapes in effect. A link
  # title consists of either a sequence of zero or more characters between
  # straight double-quote characters ("), including a " character only if it is
  # backslash-escaped, or a sequence of zero or more characters between straight
  # single-quote characters ('), including a ' character only if it is
  # backslash-escaped, or a sequence of zero or more characters between matching
  # parentheses ((...)), including a ) character only if it is
  # backslash-escaped.
  elsif (m/\G\[(.+?)\]\(\s*(?:&lt;([^ \n]*)&gt;|([^ [:cntrl:]]+))?(?:\s+(?:"(.*)"|'(.*)'|\((.*)\)))?\s*\)/scg) {
    my ($link, $destination, $title) = ($1, $2||$3, $4||$5||$6);
    # warn("Destination: ($1, $2||$3, $4||$5||$6)\n");
    ...
    return qq(<a href="$destination" title="$title">$link</a>) if $title;
