...
$ModulesDescription .= '<p>$Id: headlines.pl,v 1.1 2004/02/22 13:19:09 as Exp $</p>';
...
sub HeadlinesPrint {
  ...
  if (@pages) {
	...
	my ($date, $title) = ($1, $2);
	print '<span class="date">' . $date . '</span>' unless $date eq $current_date;
	$current_date = $date;
	print ScriptLink($page, $title, 'headline');
      }
    }
    print '</div>';
  }
