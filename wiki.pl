#! /usr/bin/perl
# Version       $Id: wiki.pl,v 1.912 2009/03/27 21:54:41 as Exp $
...
$VERSION=(split(/ +/, q{$Revision: 1.912 $}))[1]; # for MakeMaker
...
sub InitVariables {  # Init global session variables for mod_perl!
  $WikiDescription = $q->p($q->a({-href=>'http://www.oddmuse.org/'}, 'Oddmuse'),
    ...
    . $q->p(q{$Id: wiki.pl,v 1.912 2009/03/27 21:54:41 as Exp $});
...
sub GrepFiltered { # grep is so much faster!!
  my ($string, @pages) = @_;
  return @pages unless GetParam('grep', $UseGrep);
