#! /usr/bin/perl
# Version       $Id: wiki.pl,v 1.927 2009/08/15 22:11:14 as Exp $
...
$VERSION=(split(/ +/, q{$Revision: 1.927 $}))[1]; # for MakeMaker
...
sub InitVariables {  # Init global session variables for mod_perl!
  $WikiDescription = $q->p($q->a({-href=>'http://www.oddmuse.org/'}, 'Oddmuse'),
    ...
    . $q->p(q{$Id: wiki.pl,v 1.927 2009/08/15 22:11:14 as Exp $});
...
sub SearchRegexp {
  ...
  $regexp =~ s/\\s/[[:space:]]/g;
  eval { $regexp = qr/$regexp/i; }; # handle invalid regular expressions
